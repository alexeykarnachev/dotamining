...
#                      "team.hero_4", "team.hero_5", "opponent.hero_1", "opponent.hero_2",
#                      "opponent.hero_3", "opponent.hero_4", "opponent.hero_5"

# minDate - min. date of the played matches

# minGames - min. team's games. If team has less games, that minGames-value, function will
# return nothing

# parsePlayers - the logical. Need parse players in every match (parsing will be 20-times longer)


parse.teamsMatchesResults = function(team.id, features, minDate, minGames, parsePlayers){
  ...
  for(page in 1:pages){
    ...
    if(page > 1){
      url = paste0("http://dotabuff.com/esports/teams/", team.id, "/matches?page=", page)
      doc = getNodeSet(htmlParse(url), "//tbody/tr[not(@class)]")
    }
    ...
    for(iRow in seq(along.with = doc)){
      ...
      team.heroes = gsub(unlist(xmlApply(getNodeSet(iRow.doc, "td[4]/div/a[@href]"), function(x) xmlAttrs(x))), pattern = "/heroes/", replacement = "" )
      opponent.heroes = gsub(unlist(xmlApply(getNodeSet(iRow.doc, "td[5]/div/a[@href]"), function(x) xmlAttrs(x))), pattern = "/heroes/", replacement = "" )
      ...
      match.id = gsub(xmlAttrs(getNodeSet(iRow.doc, "td[1]/a")[[1]])['href'], pattern = "/matches/", replacement = "")
      match.result = xmlAttrs(getNodeSet(iRow.doc, "td[1]/a")[[1]])['class']
      opponent.id = gsub(xmlAttrs(getNodeSet(iRow.doc, "td[2]/a")[[1]])['href'], pattern = "/esports/teams/", replacement = "")
      opponent.name = xmlValue(getNodeSet(iRow.doc, "td[2]/a")[[1]])
      
      rownames = append(rownames, match.id)
      row = c(match.date, match.league, team.name, team.id, opponent.name, opponent.id, match.result, match.time, team.heroes, opponent.heroes)
  ...
  if(!is.null(nrow(team.table)) && enoughGames  && !is.null(team.table)){
    ...
    if(parsePlayers){
      ...
      for(match.id_i in 1:nrow(team.table)){
        ...
        team1.names = unlist(xmlApply(getNodeSet(doc, "//a[@class='player-radiant']"), xmlValue))
        team2.names = unlist(xmlApply(getNodeSet(doc, "//a[@class='player-dire']"), xmlValue))
        team12names = cbind(team1.names, team2.names)
        
        team1.ids = unlist(str_extract_all(unlist(xmlApply(getNodeSet(doc, "//a[@class='player-radiant']"), xmlAttrs)), "([[:digit:]]+)"))
        team2.ids = unlist(str_extract_all(unlist(xmlApply(getNodeSet(doc, "//a[@class='player-dire']"), xmlAttrs)), "([[:digit:]]+)"))
        team12ids = cbind(team1.ids, team2.ids)
        
        teams.ids = unlist(str_extract_all(unlist(unique(xmlApply(getNodeSet(doc, "//header[@style='vertical-align: middle']/a[@ href]"), function(x) xmlAttrs(x)))), "([[:digit:]]+)"))
        ...
        playersNames.row = c(team.players.names, team.players.ids, opponent.players.names, opponent.players.ids)   
      ...
      colnames(playersNames.table) = c("team.player_1", "team.player_2", "team.player_3", "team.player_4", "team.player_5",
                                       "team.player_1.id", "team.player_2.id", "team.player_3.id", "team.player_4.id", "team.player_5.id",
                                       "opponent.player_1", "opponent.player_2", "opponent.player_3", "opponent.player_4", "opponent.player_5",
                                       "opponent.player_1.id", "opponent.player_2.id", "opponent.player_3.id", "opponent.player_4.id", "opponent.player_5.id")
